...
from Shoprite.models import Store, Circular, Item, SaleItem, Units, XForYItem, FlatPriceItem, \
    FractionalReductionItem, PercentReductionItem, RangedPriceItem, Conditional, FlatReductionItem
...
def build_circular(partial_url):
    ...
    for item in item_list:
        static_item, created = Item.objects.get_or_create(name=item["name"], category=item["category"])
        ...
        if returns["bool"]:
            Conditional.objects.create(sale_item=sale_item, buy=returns["buy"], get=returns["get"])
            continue

        #  Check if sale is an x items for $y
        returns = shoprite_price_data.x_for_y(returns["price"])
        if returns["bool"]:
            XForYItem.objects.create(
                sale_item=sale_item,
        ...
        if returns["bool"]:
            FractionalReductionItem.objects.create(
                sale_item=sale_item,
        ...
        if returns["bool"]:
            ...
            FlatReductionItem.objects.create(sale_item=sale_item, price_reduction=returns["price"])

        #  Check if sale is a flat price
        returns = shoprite_price_data.flat_price(returns["price"])
        if returns["bool"]:
            FlatPriceItem.objects.create(sale_item=sale_item, price="{0:.2f}".format(float(returns["price"])))
            continue
        "\n====================================================================="
        print "Static Item: " + static_item.name + "\tCreated:" + str(created)
        print "Original price string: " + item["price"]

...
import Shoprite.models as shop
...
def build_circular(partial_url):
    ...
    store, created = shop.Store.objects.get_or_create(name="Shoprite",
                                                        address=circular_info["info"]["address"],
                                                        locale=circular_info["info"]["locale"],
                                                        phone_number=circular_info["info"]["phone_number"])
    circular = shop.Circular.objects.create(store=store.pk, start=dates["start"], end=dates["end"])
    for item in item_list:
        m_item = shop.Item.objects.get_or_create(name=item["name"])
        sale_item = shop.SaleItem.objects.create(item=m_item.pk, circular=circular.pk)
        ...
        if returns["bool"]:
            shop.Units.objects.create(item=sale_item.pk, unit=returns["units"])

        #  Check if price plus card is required
        returns = pdt.get_plus_card(returns["price"])
        shop.SaleItem.objects.filter(pk=sale_item.pk).update(card_requirement=returns["bool"])

        #  Check if sale is an x items for $y
        returns = pdt.x_for_y(returns["price"])
        if returns["bool"]:
            shop.XForYItem.objects.create(sale_item=sale_item.pk, count=returns["count"], price=returns["price"])

        #  Check if sale is a fractional reduction
        returns = pdt.fractional_reduction(returns["price"])
        if returns["bool"]:
            shop.FractionalReductionItem.objects.create(sale_item=sale_item.pk, fractional_reduction=returns["price"])

        #  Check if sale is a percent reducation
        returns = pdt.percent_reduction(returns["price"])
        if returns["bool"]:
            shop.PercentReductionItem.objects.create(sale_item=sale_item.pk, percent_reduction=returns["price"])

        #  Check if sale is a ranged price
        returns = pdt.price_range(returns["price"])
        if returns["bool"]:
            shop.RangedPriceItem.objects.create(sale_item=sale_item.pk, minimum_price=returns["price"][0],
                                                  maximum_price=returns["price"][1])
        """
        #Check if sale is a flat price reduction
        returns = pdt.flat_reduction(returns["price"])
        if returns["bool"]:
            shop.RangedPriceItem.objects.create(sale_item=sale_item.pk, price_reduction=returns["price"])
        """
        #Check if sale is a flat price
        returns = pdt.flat_price(returns["price"])
        if returns["bool"]:
            shop.FlatPriceItem.objects.create(sale_item=sale_item.pk, price=returns["price"])
